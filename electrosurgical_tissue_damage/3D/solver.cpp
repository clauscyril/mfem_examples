...
void compute_Q(Mesh &mesh, FiniteElementSpace *fespace, GridFunction &q){
    ...
    for (int i = 0; i < mesh.GetNV(); i++) {
        ...
        // if (u[0] <= -0.0159 and (abs(u[1] - 1.4e-3) < 0.4e-3 or abs(u[1] + 1.4e-3) < 0.4e-3) )
        real_t r = sqrt(u[0]*u[0] + u[1]*u[1] + u[2]*u[2]);
        if (r > 15.95e-3 and (abs(u[1] - 1.4e-3) < 0.8e-3 or abs(u[1] + 1.4e-3) < 0.8e-3) and abs(u[2]) < 0.4e-3 and u[0] < 0)
        // if (r > 15.98e-3 and abs(u[0]) > 15.98e-3)
    ...
    for (int i = 0; i < ess_tdof_list.Size(); i++){
        ...
        // if (u[0] <= -15.5e-3 and (abs(u[1] - 2e-3) < 0.5e-3 or abs(u[1] + 2e-3) < 0.5e-3))
        //     v(ess_tdof_list[i]) = 120/sqrt(2);
        // else 
        //     v(ess_tdof_list[i]) = 0;
