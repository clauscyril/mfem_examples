...
// Fonction de source qui tourne autour du bord 
real_t q_func(const Vector &x, real_t &t) {
    real_t theta = atan2(x[1], x[0]);
    real_t omega_t = 2*M_PI*0.001*t;
    if (sqrt(x[0]*x[0] + x[1]*x[1]) >= 0.48)
        return 100 * exp(-std::pow(std::fmod(omega_t - theta,2*M_PI) - M_PI,2)/(2*std::pow(M_PI/8,2)));
    return 0;
}
